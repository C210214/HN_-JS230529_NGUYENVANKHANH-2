...
function ShoppingCart() {
    ...
    const productRemove = (product) => {
        setCartProduct((oldProduct) => {
            const productsIndex = oldProduct.findIndex((item) => item.id === product.id);
            if (productsIndex !== -1) {
                oldProduct.splice(productsIndex, 1);
            }
            return [...oldProduct];
        });
    };
    ...
    return (
        <div className="App">
            ...
            <div className="navbar">
                <h3 className="logo">My Store</h3>
                <h2>
                    To Spend <span className="navbar-title">$128,000,000,000</span> You have money
                </h2>
                <button className="btn shopping-cart-btn" onClick={() => setCartVisible(true)}>
                    <i className="fa-solid fa-bag-shopping" style={{ fontSize: "24px" }}></i>

                    <span className="product-count">{cartProduct.length}</span>
                </button>
            </div>
            ...
            <main>
                ...
                <div className="products">
                    {dataProduct.map((product, id) => (
                        <Product key={id} elementProduct={product} addToCart={addToCart} />
                    ))}
                </div>
            </main>
        </div>
